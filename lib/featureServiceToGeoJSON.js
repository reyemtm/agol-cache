...
async function featureServiceToGeoJSON(featureServiceUrl, options, callback) {
  ...
  for (let i = 0; i < layers.length; i++) {
    const l = layers[i];
    const domainFields = [];
    const layerDefinition = await fetch(`${featureServiceUrl}${l.id}/?f=json${(config.token) ? `&token=${config.token}` : ""}`)
    .then(res => res.json())
  ...
  const asyncHandleLayers = async () => {
    ...
    if (options.layerByLayer) {
      ...
      for (let i = 0; i < layers.length; i++) {
        ...
        const { count } = await fetch(`${featureServiceUrl}${l.id}/query?f=json&where=1=1&returnCountOnly=true${(config.token) ? `&token=${config.token}` : ""}`).then(res => res.json())
        if (!count) {
        ...
        try {
          ...
          await getFeatures(`${featureServiceUrl}${l.id}/`, l.name, config, l.id, l.geometryType, l.renderer, l.domainFields, layers);
          total = total - 1;
          if (!total) {
            spinner.stop()
            if (callback) {
    ...
    /*-------------*/
      ...
      layers.forEach(async l => {
        ...
        try {
          ...
          await getFeatures(`${featureServiceUrl}${l.id}/`, l.name, config, l.id, l.geometryType, l.renderer, l.domainFields);
          total = total - 1;
          if (!total) {
            spinner.stop();
            if (callback) {
...
async function getFeatures(layerUrl, layerName, opts, id, geometryType, renderer, domainFields, layers) {
  if (opts.debug) {
    logger.info("---")
  ...
  if (!opts.esriIdField || opts.hasAttachments) {
    ...
    try {
      ...
      if (!idFieldObject.length) {
        ...
        if (!opts.silent) spinner.succeed("Found esriIdField for " + layerName + ": " + idFieldObject[0].name).start()
      }
