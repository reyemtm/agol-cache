...
async function getFeatures(layerUrl, layerName, opts, id) {
  ...
  while (!completed) {
    ...
    try {
      ...
      if (geojson.error && ids.end > END) {
        completed = true
      }
      if ((!geojson || !geojson.features || !geojson.features.length) &&  ids.end > END){
        completed = true;
      } else {
...
async function queryAGOL(url, idField, start, end, opts, layerName) {
  const query = `${url}query?where=${idField}+between+${start}+and+${end}&outFields=*&f=geojson${(opts.token) ? `&token=${opts.token}` : ""}`;
  let data = {};
  try {
