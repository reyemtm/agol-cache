...
const retryOptions = {
  ...
  onRetryAttempt: err => {
    ...
    spinner.text = `Retry attempt ${Number(cfg.currentRetryAttempt)}`
  }
}
    ...
    if (options.layerByLayer) {
      ...
      layers.forEach(async l => {
        ...
        try {
          ...
          spinner.text = "Error on Layer " + total.toString()
          if (opts.debug) console.log({fn: "getFeatures", error});
          total = total - 1;
        }
        finally {
  ...
  if (!opts.esriIdField || opts.hasAttachments) {
    try {
      ...
      const query = await axios({
        url: `${layerUrl}?f=pjson&token=${(opts.token) ? `${opts.token}` : ""}`,
        timeout: 10000,
        raxConfig: retryOptions
      });
