  ...
  const asyncHandleLayers = async () => {
    ...
    if (options.layerByLayer) {
      ...
      for (let i = 0; i < layers.length; i++) {
        ...
        if (!count) {
          if (config.debug) logger.warn("Error with " + l.name + " getting total layer count!");
        }

        layers[i]["count"] = count
        
        if (!config.silent) spinner.info("Layer total features: " + count)
        ...
        try {
          ...
          await getFeatures(`${featureServiceUrl}${l.id}/`, l.name, config, l.id, l.geometryType, l.renderer, layers);
          total = total - 1;
          if (!total) {
            spinner.stop()
            if (callback) {
...
async function getFeatures(layerUrl, layerName, opts, id, geometryType, renderer, layers) {
  if (opts.debug) {
    logger.info("---")
